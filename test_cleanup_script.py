#!/usr/bin/env python3
"""
Script to test the cleanup_and_bold_first_words functionality on our test files
with a 20 character limit and stopping at space, newline, or "[" character
"""
...
def bold_first_word(input_file):
    ...
    try:
        ...
        print(f"Applied bolding to: {input_file}")
...
def main():
    ...
    print(f"Found {len(txt_files)} .txt test files in {directory}")
    ...
    for txt_file in txt_files:
        try:
            ...
            if is_already_bolded(txt_file):
                print(f"Skipping already bolded file: {os.path.basename(txt_file)}")
